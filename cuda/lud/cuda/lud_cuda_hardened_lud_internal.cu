...
{
  __shared__ float peri_row[BLOCK_SIZE][BLOCK_SIZE*3]; //L
  __shared__ float peri_col[BLOCK_SIZE][BLOCK_SIZE*3]; //L
  __shared__ float sum[BLOCK_SIZE][BLOCK_SIZE*3]; // added by Lishan. 
      // Because we want to compare the result 'sum' across the 3 copies, this result needs to be shared.

  int cur_copy = threadIdx.x / BLOCK_SIZE; // added by Lishan

  int i;
//  float sum; // commented by Lishan

  int global_row_id = offset + (blockIdx.y+1)*BLOCK_SIZE;
  int global_col_id = offset + (blockIdx.x+1)*BLOCK_SIZE;

  peri_row[threadIdx.y][threadIdx.x] = m[(offset+threadIdx.y)*matrix_dim+global_col_id+threadIdx.x%BLOCK_SIZE]; // modified by Lishan
  peri_col[threadIdx.y][threadIdx.x] = m[(global_row_id+threadIdx.y)*matrix_dim+offset+threadIdx.x%BLOCK_SIZE]; // modified by Lishan

  //peri_row[threadIdx.y][threadIdx.x] = m[(offset+threadIdx.y)*matrix_dim+global_col_id+threadIdx.x]; // original code
  //peri_col[threadIdx.y][threadIdx.x] = m[(global_row_id+threadIdx.y)*matrix_dim+offset+threadIdx.x]; // original code

  __syncthreads();
  
  sum[threadIdx.y][threadIdx.x] = 0; // modified by Lishan
  // sum = 0; // original code
  for (i=0; i < BLOCK_SIZE; i++)
    sum[threadIdx.y][threadIdx.x] += peri_col[threadIdx.y][i+BLOCK_SIZE*cur_copy] * peri_row[i][threadIdx.x]; // modified by Lishan
//    sum[threadIdx.y][threadIdx.x] += peri_col[threadIdx.y][i+BLOCK_SIZE*cur_copy] * peri_row[i][threadIdx.x]; // original code
  ...
  if (cur_copy == 0)
  {
    if (sum[threadIdx.y][threadIdx.x] != sum[threadIdx.y][threadIdx.x+BLOCK_SIZE])
      sum[threadIdx.y][threadIdx.x] = sum[threadIdx.y][threadIdx.x+BLOCK_SIZE*2];
    m[(global_row_id+threadIdx.y)*matrix_dim+global_col_id+threadIdx.x] -= sum[threadIdx.y][threadIdx.x];
  }
  // END of Lishan adding
//  m[(global_row_id+threadIdx.y)*matrix_dim+global_col_id+threadIdx.x] -= sum; // original code
...
{
  int i=0;
  dim3 dimBlock(BLOCK_SIZE*3, BLOCK_SIZE); // modified by Lishan
//  dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE); // original code
