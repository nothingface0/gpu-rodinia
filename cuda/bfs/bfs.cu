...
#ifdef COMPARE
	fprintf(stderr, " <results_output_file>");
#endif
	fprintf(stderr, "\n");
}
////////////////////////////////////////////////////////////////////////////////
...
void BFSGraph(int argc, char **argv)
{
...
#ifdef COMPARE
	results_filepath = argv[2];
	FILE *fp_res = NULL;
	if ((fp_res = fopen(results_filepath, "r")) == 0)
	{
		printf("No results file %s found\n", results_filepath);
		exit(1);
	}
	fclose(fp_res);
#endif
	...
	FILE *fpo = fopen(results_filepath, "r");
	char *line = NULL;
	bool error = false;
	size_t len = 0;
#endif
	char res[1000];
#ifdef OUTPUT
	FILE *f_out = fopen("results_out.txt", "w+");
#endif
	for (int i = 0; i < no_of_nodes; i++)
	{
