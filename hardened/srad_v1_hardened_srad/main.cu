...
#include "correcting_kernel.cu" // added by Lishan
#include "graphics.c"
#include "resize.c"
#include "timer.c"
...
//	read_graphics(	"../../../data/srad/image.pgm",
	read_graphics( "/sciclone/pscr/lyang11/NVBitFI/nvbit_release/tools/nvbitfi/nvbit_data/srad_v1/image.pgm",
								image_ori,
								image_ori_rows,
								image_ori_cols,
								1);
	...
	cudaMalloc((void **)&d_I, mem_size); // modified by Lishan
	...
	cudaMalloc((void **)&d_sums, mem_size); // L
	cudaMalloc((void **)&d_sums2, mem_size); // L

	// allocate memory for derivatives
	cudaMalloc((void **)&d_dN, mem_size*3);														// 
	cudaMalloc((void **)&d_dS, mem_size*3);														// 
	cudaMalloc((void **)&d_dW, mem_size*3);													// 
	cudaMalloc((void **)&d_dE, mem_size*3);														// 

	// allocate memory for coefficient on DEVICE
	cudaMalloc((void **)&d_c, mem_size*3);														// 
	...
	// Lishan's note
	// output: d_dN, d_dS, d_dW, d_dE, d_c size: 16384
	// cta size: 512. So, we have to dup ctas.
	// ori: blocks.y = 1
	blocks.y = 3;
	srad<<<blocks, threads>>>(lambda, Nr, Nc, Ne, d_iN, d_iS, d_jE, d_jW, d_dN, d_dS, d_dW, d_dE, q0sqr, d_c, d_I);
	blocks.y = 1;
	...
	checkCUDAError("srad");
 	...
 	dim3 dimGridHardening1(Ne/256); 


	check_correctness<<<dimGridHardening1, dimBlockHardening>>>(d_dN, Ne);
	check_correctness<<<dimGridHardening1, dimBlockHardening>>>(d_dS, Ne);
	check_correctness<<<dimGridHardening1, dimBlockHardening>>>(d_dW, Ne);
	check_correctness<<<dimGridHardening1, dimBlockHardening>>>(d_dE, Ne);
	check_correctness<<<dimGridHardening1, dimBlockHardening>>>(d_c, Ne);
