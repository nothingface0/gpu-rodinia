...
{   
	...
	*(m_cuda + blockIdx.y*6400+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) = *(a_cuda + Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) / *(a_cuda + Size*t+t);
	// *(m_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) = *(a_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) / *(a_cuda+Size*t+t); // original code
}
...
void ForwardSub()
{
	...
	cudaMalloc((void **) &m_cuda, Size * Size * sizeof(float)*3); // Modified by Lishan
	...
	for (t=0; t<(Size-1); t++) {
		...
		// dimGrid.x = 1
		// dimBlock.x = 512
		// m_cuda, output, size=6400
		// a_cuda, input, size=6400
		...
		//Fan1<<<dimGrid,dimBlock>>>(m_cuda,a_cuda,Size,t); // original code
