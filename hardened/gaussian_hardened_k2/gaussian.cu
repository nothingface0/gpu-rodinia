...
__global__ void Fan2(float *m_cuda, float *a_cuda, float *b_cuda,int Size, int j1, int t)
{
        ...
        a_cuda[Size*(xidx+1+t)+(yidx+t) + blockIdx.z * 6400 ] =         a_cuda[Size*(xidx+1+t)+(yidx+t) + blockIdx.z * 6400 ] - m_cuda[Size*(xidx+1+t)+t] * a_cuda[Size*t+(yidx+t) + blockIdx.z * 6400];
        //a_cuda[xidx+1+t][yidx+t] -= m_cuda[xidx+1+t][t] * a_cuda[t][yidx+t];
        if(yidx == 0){
                //printf("blockIdx.x:%d,threadIdx.x:%d,blockIdx.y:%d,threadIdx.y:%d,blockDim.x:%d,blockDim.y:%d\n",blockIdx.x,threadIdx.x,blockIdx.y,threadIdx.y,blockDim.x,blockDim.y);
                //printf("xidx:%d,yidx:%d\n",xidx,yidx);
                b_cuda[xidx+1+t + blockIdx.z * 80] =  b_cuda[xidx+1+t + blockIdx.z * 80]   - m_cuda[Size*(xidx+1+t)+(yidx+t)] * b_cuda[t + blockIdx.z * 80];
        }
}
...
void ForwardSub()
{
	...
	cudaMalloc((void **) &a_cuda, Size * Size * sizeof(float)*3); // modified by Lishan
	
	cudaMalloc((void **) &b_cuda, Size * sizeof(float)*3);	// modified by Lishan
	...
	for (t=0; t<(Size-1); t++) {
		...
		// Lishan's note
		// Size = 80
		// a_cuda: 80x80=6400  output&input
		// b_cuda: 80  output&input
		// m_cuda: 80x80=6400  input
		// grid: (20, 20) block: (4,4)

		dim3 dimBlockHardening(256);
                dim3 dimGridHardening(6400/256);

        	prepare_dup<<<dimGridHardening, dimBlockHardening>>>(a_cuda, 6400);
		dimGridHardening.x = 1;
        	prepare_dup<<<dimGridHardening, dimBlockHardening>>>(b_cuda, 80);

		dimGridXY.z = 3;
		Fan2<<<dimGridXY,dimBlockXY>>>(m_cuda,a_cuda,b_cuda,Size,Size-t,t);
		dimGridXY.z = 1;

        	check_correctness<<<dimGridHardening, dimBlockHardening>>>(b_cuda, 80);
		dimGridHardening.x = 6400/256;
        	check_correctness<<<dimGridHardening, dimBlockHardening>>>(m_cuda, 6400);
		/* END of Lishan modify */

		// Fan2<<<dimGridXY,dimBlockXY>>>(m_cuda,a_cuda,b_cuda,Size,Size-t,t); // original code
		cudaThreadSynchronize();
		checkCUDAError("Fan2");
	}
	// end timing kernels
