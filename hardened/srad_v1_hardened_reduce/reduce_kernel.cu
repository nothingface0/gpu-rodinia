...
//    if (blockIdx.y != 0) return;
	...
	if(nf == NUMBER_THREADS){
		...
		if(tx==(NUMBER_THREADS-1)){											// block result stored in global memory
			d_sums[bx*d_mul*NUMBER_THREADS+16384*blockIdx.y] = d_psum[tx];
			d_sums2[bx*d_mul*NUMBER_THREADS+16384*blockIdx.y] = d_psum2[tx];
		}
	}
		...
		// for not full block (last block)
			...
			if(tx==(df-1)){										//
				...
				for(i=(bx*NUMBER_THREADS)+df; i<(bx*NUMBER_THREADS)+nf; i++){						//
					d_psum[tx] = d_psum[tx] + d_sums[i+16384*blockIdx.y];
					d_psum2[tx] = d_psum2[tx] + d_sums2[i+16384*blockIdx.y];
				}
				// final sumation by last thread in every block
				d_sums[bx*d_mul*NUMBER_THREADS+16384*blockIdx.y] = d_psum[tx];
				d_sums2[bx*d_mul*NUMBER_THREADS+16384*blockIdx.y] = d_psum2[tx];
