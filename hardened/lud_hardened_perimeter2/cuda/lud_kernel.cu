...
lud_perimeter(float *m, int matrix_dim, int offset)
{
  ...
  if (threadIdx.x < BLOCK_SIZE) {
    ...
    for (i=0; i < BLOCK_SIZE/2; i++){
      dia[i][idx]=m[array_offset+idx + 16384*blockIdx.y];
      array_offset += matrix_dim;
    }
    
    array_offset = offset*matrix_dim+offset;
    for (i=0; i < BLOCK_SIZE; i++) {
      peri_row[i][idx]=m[array_offset+(blockIdx.x+1)*BLOCK_SIZE+idx + 16384*blockIdx.y];
      array_offset += matrix_dim;
    }
    ...
    for (i=0; i < BLOCK_SIZE; i++) {
      peri_col[i][idx] = m[array_offset+idx + 16384*blockIdx.y];
      array_offset += matrix_dim;
    }
  ...
  if (threadIdx.x < BLOCK_SIZE) { //peri-row
    ...
    array_offset = (offset+1)*matrix_dim+offset;
    for(i=1; i < BLOCK_SIZE; i++){
      m[array_offset+(blockIdx.x+1)*BLOCK_SIZE+idx + 16384*blockIdx.y] = peri_row[i][idx];
      array_offset += matrix_dim;
    }
  } else { //peri-col
    idx=threadIdx.x - BLOCK_SIZE;
    array_offset = (offset+(blockIdx.x+1)*BLOCK_SIZE)*matrix_dim+offset;
    for(i=0; i < BLOCK_SIZE; i++){
      m[array_offset+idx + 16384*blockIdx.y] =  peri_col[i][idx];
      array_offset += matrix_dim;
    }
  }
}
...
void lud_cuda(float *m, int matrix_dim)
{
  ...
  for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
	...
	dim3 dimPerimeter( (matrix_dim-i)/BLOCK_SIZE-1, 3);
      lud_perimeter<<<dimPerimeter, BLOCK_SIZE*2>>>(m, matrix_dim, i);
	...
	/* END of Lishan modify */
      ...
      // lud_perimeter<<<(matrix_dim-i)/BLOCK_SIZE-1, BLOCK_SIZE*2>>>(m, matrix_dim, i); // original code
