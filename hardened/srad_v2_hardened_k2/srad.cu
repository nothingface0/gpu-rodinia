...
runTest( int argc, char** argv) 
{
    ...
    cudaMalloc((void**)& J_cuda, sizeof(float)* size_I*3); // modified by Lishan
 ...
 for (iter=0; iter< niter; iter++){     
	...
	dim3 dimBlockHardening(256);
        ...
        prepare_dup<<<dimGridHardening, dimBlockHardening>>>(J_cuda, size_I);
	...
	dimGrid.x = block_x;
        ...
        check_correctness<<<dimGridHardening, dimBlockHardening>>>(J_cuda, size_I);
	...
	// srad_cuda_2<<<dimGrid, dimBlock>>>(E_C, W_C, N_C, S_C, J_cuda, C_cuda, cols, rows, lambda, q0sqr); // original code 
