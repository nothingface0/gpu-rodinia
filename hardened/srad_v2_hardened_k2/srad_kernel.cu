...
{
	//block id
	int bx = blockIdx.x%16;  // L
	int cur_copy = blockIdx.x / 16; // L
    int by = blockIdx.y;
    ...
    //load data to shared memory
	temp[ty][tx]      = J_cuda[index+cur_copy*65536];
	...
	if ( bx == gridDim.x/3 - 1 ){
	east_c[ty][tx] = C_cuda[cols * BLOCK_SIZE * by + BLOCK_SIZE * ( gridDim.x/3 - 1) + cols * ty + BLOCK_SIZE-1];
	}
   ...
   J_cuda[index+cur_copy*65536] = c_cuda_result[ty][tx];
