...
vectorAdd(const float *A, const float *B, float *C, int numElements)
{
    ...
    if (i < numElements)
    {
        C[i+1049087*blockIdx.y] = A[i] + B[i]; // Modified by Lishan
        // C[i] = A[i] + B[i]; // original code
    }
}
...
main(void)
{
    ...
    float *d_C = NULL;
    err = cudaMalloc((void **)&d_C, size*3); // L
    ...
    dim3 dimGrid (blocksPerGrid, 3);

    vectorAdd<<<dimGrid, threadsPerBlock>>>(d_A, d_B, d_C, numElements);


    dim3 dimBlockHardening(256); 
    dim3 dimGridHardening(numElements/256);
    ...
    // vectorAdd<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C, numElements); // original code
