			...
			int    *block_deltas) 
{
	...
	// block ID
    int bx = blockIdx.x % 2;
    int cur_copy = blockIdx.x / 2;
	const unsigned int block_id = gridDim.x/3*blockIdx.y+bx;
	// point/thread ID  
...
*/ // tmp comment
    ...
    // Lishan's note: GPU_DELTA_REDUCTION is not enabled.
		if (membership[point_id+cur_copy*800] != index) {
			deltas[threadIdx.x] = 1;
		}
#endif
*/ // end of tmp comment
		/* assign the membership to object point_id */
		membership[point_id+cur_copy*800] = index;
        // printf ("membership %d %d\n", point_id, index);
	}

    /* tmp comment
#ifdef GPU_DELTA_REDUCTION
    // Lishan's note: this part is not enabled.
	...
	if(threadIdx.x == 0) {
		block_deltas[blockIdx.y * gridDim.x/3 + bx + cur_copy*800] = deltas[0];
...
*/ // end of tmp comment
    ...
    // Lishan't note: GPU_NEW_CENTER_REDUCTION is not enabled.
	...
	*** 	
	...
	if(threadIdx.x < nfeatures * nclusters) {
		...
		// ***
		block_clusters[(blockIdx.y*gridDim.x/3 + bx) * nclusters * nfeatures + threadIdx.x] = accumulator;
	}
#endif
    */ // end of tmp comment
