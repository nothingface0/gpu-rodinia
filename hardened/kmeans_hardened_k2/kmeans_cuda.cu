...
{	
	...
	cudaMalloc((void**) &membership_d, npoints*sizeof(int)*3); // Lishan modify
	cudaMalloc((void**) &clusters_d, nclusters*nfeatures*sizeof(float)); // Lishan Modify

	
#ifdef BLOCK_DELTA_REDUCE
    // Lishan's note: not used.
...
//	time_kernel = omp_get_wtime();
    ...
    // npoints = 800, nfeatures = 34, 800*34=27200
    // num_blocks_perdim = 2, num_threads_perdim = 16
    // membership, size=npoints = 800 
    // cluster_d, size=ncluster*nfeatures = 5*34=170
    // block_deltas_d, num_blocks_perdim * num_blocks_perdim = 2*2=4, but this is not used.
    // block_clusters_d, not used.

    dim3 dimBlockHardening(256);
    dim3 dimGridHardening(800/256 + 1);
    grid.x = 2*3;
    ...
    check_correctness_int<<<dimGridHardening, dimBlockHardening>>>(membership_d, 800);
 //   dimGridHardening.x = 1;
//    check_correctness<<<dimGridHardening, dimBlockHardening>>>(clusters_d, 170);
//    check_correctness_int<<<dimGridHardening, dimBlockHardening>>>(block_deltas_d, 4);
...
#endif
#ifdef BLOCK_DELTA_REDUCE
    // Lishan's note: BLOCK_DELTA_REDUCE not used.
