...
void bpnn_train_cuda(BPNN *net, float *eo, float *eh)
{
  ...
  cudaMalloc((void**) &input_hidden_cuda, (in + 1) * (hid + 1) * sizeof(float)*3); // L size:139281
  ...
  // Lishan's note.
  // output: output_hidden_cuda, hidden_partial_sum 
  // but, output_hidden_cuda is not used. they are writing to input_hidden_cuda
  // threads size: 16x16=256.
  ...
  cudaMalloc((void**) &input_prev_weights_cuda, (in + 1) * (hid + 1) * sizeof(float)*3); // L. size=139281
  ...
  // Lishan's note.
  // output: w(input_hidden_cuda), oldw(input_prev_weights_cuda) 
  
  dim3 dimBlockHardening(256); 
  dim3 dimGridHardening(139281/256+1); 

  prepare_dup<<<dimGridHardening, dimBlockHardening>>>(input_hidden_cuda, 139281);
  prepare_dup<<<dimGridHardening, dimBlockHardening>>>(input_prev_weights_cuda, 139281);

  grid.x = 3;
  bpnn_adjust_weights_cuda<<< grid, threads >>>(hidden_delta_cuda, hid,	input_cuda,
					in, input_hidden_cuda,input_prev_weights_cuda);
  grid.x = 1;
 
  check_correctness<<<dimGridHardening, dimBlockHardening>>>(input_hidden_cuda, 139281);

  check_correctness<<<dimGridHardening, dimBlockHardening>>>(input_prev_weights_cuda, 139281);
