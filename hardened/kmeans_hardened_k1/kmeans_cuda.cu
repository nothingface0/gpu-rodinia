...
{	
	...
	cudaMalloc((void**) &feature_d, npoints*nfeatures*sizeof(float)*3);
		
	/* invert the data array (kernel execution) */	
    /* START of Lishan modify */
    // npoints = 800, nfeatures = 34
    // num_blocks = 4, num_threads = 256
    // data size = 27200
    dim3 dimBlockHardening(num_blocks, 3);
    dim3 dimGridHardening(800*34/256 + 1);


	invert_mapping<<<dimBlockHardening,num_threads>>>(feature_flipped_d,feature_d,npoints,nfeatures);
    dimBlockHardening.y = 1;
    dimBlockHardening.x = 256;
    check_correctness<<<dimGridHardening, dimBlockHardening>>>(feature_d, 27200);


    /* END of Lishan modify */
		
	// invert_mapping<<<num_blocks,num_threads>>>(feature_flipped_d,feature_d,npoints,nfeatures); // original code
