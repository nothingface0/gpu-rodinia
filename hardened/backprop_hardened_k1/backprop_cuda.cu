...
void bpnn_train_cuda(BPNN *net, float *eo, float *eh)
{
  ...
  cudaMalloc((void**) &hidden_partial_sum, num_blocks * WIDTH * sizeof(float)*3); // L size: 8192
  ...
  bpnn_layerforward_CUDA<<< grid, threads1 >>>(input_cuda,
	                                          output_hidden_cuda,
											  input_hidden_cuda,
											  hidden_partial_sum,
											  in,
  ...
  dim3 dimGridHardening1(139281/256+1); 


  check_correctness<<<dimGridHardening1, dimBlockHardening>>>(input_hidden_cuda, 139281);

  dim3 dimGridHardening2(8192/256);
  check_correctness<<<dimGridHardening2, dimBlockHardening>>>(hidden_partial_sum, 8192);
