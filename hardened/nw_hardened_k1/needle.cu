...
*/	
	...
	cudaMalloc((void**)& matrix_cuda, sizeof(int)*size*3); // modified by Lishan
	// cudaMalloc((void**)& matrix_cuda, sizeof(int)*size); // original code
  ...
  gettimeofday(&tv_kernel_start, NULL);
	...
	for( int i = 1 ; i <= block_width ; i++){
		...
		// Lishan's note
		// referrence_cuda: input
		// matrix_cuda: input + output. size=83521
		// BLOCK_SIZE=16
		dim3 dimBlockHardening(256); // added by Lishan
		dim3 dimGridHardening(83521/256+1); // added by Lishan

		// Lishan's note
		// This prepare_dup is needed, otherwise output is wrong.
		prepare_dup<<<dimGridHardening, dimBlockHardening>>>(matrix_cuda, 83521);
		dimGrid.y = 3;
		needle_cuda_shared_1<<<dimGrid, dimBlock>>>(referrence_cuda, matrix_cuda
		                                      ,max_cols, penalty, i, block_width); 

		check_correctness<<<dimGridHardening, dimBlockHardening>>>(matrix_cuda,83521);
