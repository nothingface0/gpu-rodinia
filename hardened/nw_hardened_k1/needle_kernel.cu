...
/* START of Lishan add */
__global__ void prepare_dup(int* a, int size)
{
...
__global__ void check_correctness(int* result, int size)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid >= size) return;
			  ...
			  int block_width) 
{
 ...
 for( int m = BLOCK_SIZE - 2 ; m >=0 ; m--){
  ...
  for ( int ty = 0 ; ty < BLOCK_SIZE ; ty++)
  matrix_cuda[83521*blockIdx.y + index + ty * cols] = temp[ty+1][tx+1]; // modified by Lishan
  // matrix_cuda[index + ty * cols] = temp[ty+1][tx+1]; // original code
}
