...
) 
{
  ...
  int cur_copy = blockIdx.x / BLOCK_SIZE;
//  if (cur_copy != 0) return;
  ...
  else if ( bx == gridDim.x/3 - 1 ){
  east[ty][tx] = J_cuda[cols * BLOCK_SIZE * by + BLOCK_SIZE * ( gridDim.x/3 - 1) + cols * ty + BLOCK_SIZE-1];
  }
  ...
  __syncthreads();
   ...
   if ( ty == 0 && tx == 0 ){ //nw
    ...
    C_cuda[index+cur_copy*65536] = temp_result[ty][tx];
	E_C[index+cur_copy*65536] = e;
	W_C[index+cur_copy*65536] = w;
	S_C[index+cur_copy*65536] = s;
	N_C[index+cur_copy*65536] = n;
}
